...
from hoa.models.auth_method import AuthMethod, OAuth2Auth, PasskeyAuth, PasswordAuth, TokenAuth
...
class AuthMethodService:
    ...
    def get_by_id(self, auth_method_id: UUID) -> AuthMethod | None:
        """
    ...
    def get_user_auth_methods(self, user_id: UUID, enabled_only: bool = False) -> list[AuthMethod]:
        ...
        if enabled_only:
            q = q.filter(AuthMethod.enabled)

        return q.all()

    def get_passkey_by_credential_id(self, credential_id: str) -> PasskeyAuth | None:
        """
    ...
    def get_user_passkeys(self, user_id: UUID, rp_id: str | None = None) -> list[PasskeyAuth]:
        """
    ...
    def approve(
        ...
        auth_method.approved_at = datetime.now(UTC) if approved else None
    ...
    def get_pending_approvals(self, limit: int = 50) -> list[AuthMethod]:
        ...
        return self.db.query(AuthMethod).filter(
            AuthMethod.requires_approval,
            not AuthMethod.approved,
        ).limit(limit).all()
    ...
    def enable(self, auth_method_id: UUID) -> AuthMethod | None:
        """Enable an auth method."""
        return self.toggle_enabled(auth_method_id, True)

    def disable(self, auth_method_id: UUID) -> AuthMethod | None:
        """Disable an auth method."""
        return self.toggle_enabled(auth_method_id, False)

    def get_by_credential_id(self, credential_id: str) -> PasskeyAuth | None:
        """Alias for get_passkey_by_credential_id."""
        return self.get_passkey_by_credential_id(credential_id)

    def update_sign_count(self, auth_method_id: UUID, new_sign_count: int) -> PasskeyAuth | None:
